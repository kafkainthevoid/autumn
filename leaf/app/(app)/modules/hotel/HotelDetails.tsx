...
import { ChevronDownIcon, CopyIcon, Globe2Icon, MapPinIcon, PhoneIcon } from "lucide-react"
...
import { HotelVm } from "./services/HotelService"
...
export const GOOGLE_SEACH_URL = "https://www.google.com/maps/place/"
      ...
      <DialogContent className="w-2/3">
        <AlertDialogHeader>
          ...
          <DialogDescription className="text-xs">
            {hotel.address?.addressLine} {ward && `, ${ward}, `} {district && `${district}, `}
            {province}
          </DialogDescription>
        </AlertDialogHeader>
        ...
        <div className="grid grid-cols-5  items-center">
          <div className="col-span-2 flex justify-center items-center flex-col gap-3">
            <div className="text-3xl font-bold">${Math.min(...hotel.roomTypes.map((rt) => +rt.price))}</div>
          ...
          <Link
            target="_blank"
            href={GOOGLE_SEACH_URL + `${hotel.address?.addressLine},+${ward},+${district},+${province}`}
          >
          ...
          <Link target="_blank" href={""}>
            <div className="col-span-1 flex flex-col gap-3 justify-center items-center">
              ...
              <div className="text-xs text-teal-600">+ {hotel.address?.phone}</div>
            </div>
          </Link>
        </div>
        ...
        <div>
          <div className="text-xs text-zinc-500 font-semibold mb-3">Description</div>
        ...
        <div>
          <div className="text-xs text-zinc-500 font-semibold mb-3">Amenities</div>
          <div className="ml-4 text-xs grid grid-cols-4 gap-5">
            {hotel.amenity_Hotels.map((ah) => {
              return (
                <div className="flex flex-col gap-2 justify-center items-center" key={ah.amenity.id}>
                  <Image src={ah.amenity.image} alt="icon" width="30" height="30" />
                  <div className="text-xs text-zinc-500 text-center">{ah.amenity.name}I</div>
                </div>
              )
            })}
          </div>
