...
const RoomCard: FC<RoomProps> = ({ roomType, handleSelectRoom, isSelected, hotel }) => {
  return (
    <>
      <div
        className={cn(
      ...
      >
        <div className={cn("", isSelected ? "absolute top-0 left-0 bg-teal-600 text-white rounded-br-md" : "hidden")}>
          <Check className="w-6 h-6" />
        </div>
        <Image src={roomType.images[0]} alt="sf" width={0} height={0} sizes="100vw" className="w-full h-56" />
        <div className="p-2 flex flex-col flex-1 justify-between">
          <h2 className="font-medium text-xl">{roomType.name}</h2>
          <div className="block">
            <RoomDetails roomType={roomType} hotel={hotel}>
              <Button variant="link" className="text-teal-600 text-xs" size="sm">
                {/* Room details &#62; */}
                Chi tiết phòng &#62;
              </Button>
            </RoomDetails>
          ...
          >
            {/* <h2 className="font-bold block text-xs xl:text-xl">{isSelected ? "Keep Room" : "Select Room "}</h2> */}
            <h2 className="font-bold block text-xs xl:text-xl">{isSelected ? "Giữ phòng" : "Chọn phòng"}</h2>
            <div>
              <h2 className="font-bold block  text-sm xl:text-xl mr-2">
                {roomType.discount
                  ? `$ ${roomType.price} -> $ ${(roomType.price * (100 - roomType.discount.discountPercent)) / 100}`
                  : `$ ${roomType.price}`}
              </h2>
              {roomType.discount ? (
